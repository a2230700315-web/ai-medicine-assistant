...
def parse_subject_structure(table_start, table_end):
    ...
    for table_idx in range(table_start, min(table_end + 1, len(all_tables))):
        ...
        for row in table['data'][1:]:
            ...
            if detail_name and point_content:
                ...
                units[unit_name]['subunits'][subunit_name]['details'][detail_name] = {
                    'name': detail_name,
                    'points': points
                }
    ...
    for unit_name, unit_data in units.items():
        ...
        for subunit_name, subunit_data in unit_data['subunits'].items():
            # 重新生成小单元名称
            subunit_display_name = f"（{number_to_chinese(subunit_counter)}）{subunit_data['name'].replace('^[（(][一二三四五六七八九十]+[）)]', '')}"
...
# 保存解析结果
with open('parsed_subjects_structure_v2.json', 'w', encoding='utf-8') as f:
    json.dump(subjects, f, ensure_ascii=False, indent=2)

print("\n✅ 已保存解析结果到 parsed_subjects_structure_v2.json")