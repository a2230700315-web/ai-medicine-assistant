    ...
    <div className="bg-white rounded-xl shadow-lg p-4 flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">共 {filteredCases.length} 个案例</p>
        </div>
      </div>
        ...
        {filteredCases.map((case_) => {
          ...
          return (
            <button
              ...
              className="w-full p-3 bg-gray-50 rounded-xl border-2 border-gray-200 hover:border-blue-400 hover:shadow-md transition-all text-left group relative active:scale-[0.98]"
            >
              {medal && (
                <div className={`absolute top-2 right-2 px-1.5 py-0.5 rounded-full text-xs font-bold ${medal.color} bg-white shadow-sm`}>
                  {medal.icon} {medal.title}
                </div>
              )}

              <div className="flex items-start gap-3">
                <div className="w-9 h-9 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-500 mb-1">
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`font-medium ${getBMICategory(case_.bmi).color}`}>
      ...
      <div className="mt-3 pt-3 border-t border-gray-200">
        <button
          onClick={onBack}
          className="w-full py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all text-sm font-medium"
        >
          返回分类选择
        </button>
      </div>
