...
def parse_subject_structure(table_start, table_end):
    """解析科目结构，合并同一大单元下的所有小单元"""
    units = {}  # 使用字典来合并同一大单元的数据
    ...
    for table_idx in range(table_start, min(table_end + 1, len(all_tables))):
        ...
        for row in table['data'][1:]:
            ...
            # 创建小单元（如果不存在）
            if subunit_name_clean not in units[unit_name]['subunits']:
                units[unit_name]['subunits'][subunit_name_clean] = {
            ...
            if detail_name and point_content:
                ...
                units[unit_name]['subunits'][subunit_name_clean]['details'][detail_name] = {
                    'name': detail_name,
                    'points': points
                }
    ...
    for unit_name, unit_data in units.items():
        # 重新生成大单元名称
        unit_display_name = f"{unit_data['number'] if unit_data['number'] else number_to_chinese(unit_counter)}、{unit_data['name']}"
...
# 保存解析结果
with open('parsed_subjects_structure_v3.json', 'w', encoding='utf-8') as f:
    json.dump(subjects, f, ensure_ascii=False, indent=2)

print("\n✅ 已保存解析结果到 parsed_subjects_structure_v3.json")