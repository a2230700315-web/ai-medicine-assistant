...
total_topics = sum(len(unit['topics']) for unit in first_subject['units'])

print(f"\n📊 第一个大单元统计:")
print(f"  小单元数: {len(first_unit['subunits'])}")
print(f"  细目数: {sum(len(sub['details']) for sub in first_unit['subunits'])}")
print(f"  总知识点数: {sum(len(detail['points']) for sub in first_unit['subunits'] for detail in sub['details'])}")
print(f"  已生成即学即练的小单元数: {sum(1 for sub in first_unit['subunits'] if sub.get('practiceQuestions'))}")
