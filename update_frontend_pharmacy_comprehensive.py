...
for subject in all_content:
    for unit in subject['units']:
        total_all_units += 1
        total_all_subunits += len(unit['subunits'])
        for subunit in unit['subunits']:
            total_all_details += len(subunit['details'])
            for detail in subunit['details']:
                total_all_points += len(detail['points'])

print(f"  总大单元数: {total_all_units}")
print(f"  总小单元数: {total_all_subunits}")
print(f"  总细目数: {total_all_details}")
print(f"  总要点数: {total_all_points}")

print(f"\n💡 提示:")
print(f"  - 药学综合知识与技能模块的所有知识点已生成详细内容")
print(f"  - 🎉 所有7个模块的知识点内容已全部生成完成！")
