...
print(f"  仍然使用默认模板的要点数: {default_template_count}")
print(f"  需要补充的比例: {default_template_count/total_points*100:.1f}%")

print(f"\n📝 仍然使用默认模板的知识点示例 (前30个):")
for i, point in enumerate(default_points[:30]):
    print(f"\n{i+1}. [{point['subject']}] {point['unit']} > {point['subunit']} > {point['detail']}")
    print(f"   要点: {point['point']}")

# 保存需要补充的知识点列表
with open('points_still_need_update.json', 'w', encoding='utf-8') as f:
    json.dump(default_points, f, ensure_ascii=False, indent=2)

print(f"\n✅ 已保存仍然需要补充的知识点列表到 points_still_need_update.json")
